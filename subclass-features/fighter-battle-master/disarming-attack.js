...
try {
	...
	if (die && item && item.data.data.uses.value) {
		let dialog = new Promise((resolve, reject) => {
			new Dialog({
			title: "Disarming Attack: Usage Configuration",
			content: `
			<form id="use-form">
				<p>` + game.i18n.format("DND5E.AbilityUseHint", {name: "Maneuvers: Disarming Attack", type: "feature"}) + `</p>
		...
		if (!item || !item.data.data.uses.value) {
			ui.notifications.warn("Disarming Attack: No Superiority Die Remaining");
			return;
		} else {
        ...
        const ability = "str";
        const rollOptions = { request: "save", targetUuid: tactorTarget.uuid, ability: ability }
