...
try {
  ...
  if (!attacker.actor || !["mwak","rwak"].includes(args[0].itemData.data.actionType) || !args[0].hitTargets.length) return; // abort if not weapon attack or no hit targets
  if (!game.canvas.tokens.placeables.find(t => t.actor && !(t.actor?.data?.data?.details?.type?.value === "custom" && t.actor?.data?.data?.details?.type?.custom === "") && t.id !== attacker.id && t.id !== target.id && attacker.disposition === t.disposition && t.actor?.data?.data?.attributes?.hp?.value > 0 && !(t.actor?.effects?.find(e => ["Incapacitated", "Unconscious", "Paralyzed", "Petrified", "Stunned"].includes(e.data.label))) && MidiQOL.getDistance(t, target, false) <= 5)) return; // abort if no nearby ally
  if (game.combat) {
...
//return {damageRoll: `${2 * diceMult}d6[${damageType}]`, flavor: "Martial Advantage"};
