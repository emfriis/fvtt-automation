...
if (args[0].tag === "OnUse" && lastArg.failedSaves.length > 0 && args[0].macroPass === "postActiveEffects") {
    ...
    for (let i = 0; i < workflow.saveDisplayData.length; i++) {
        ...
        if (critFail) {
            let ef = tactorTarget.effects.find(i => i.data.origin === args[0].uuid);
            if (ef && !tactorTarget.data.data.traits.ci?.value.includes("poisoned")) {
                let efData = {
                ...
                let parEf = await tactorTarget.createEmbeddedDocuments("ActiveEffect", [efData]);
                ef.update({ changes: [{
                    key: "flags.dae.deleteUuid",
                    mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
                    priority: 20,
                    value: parEf[0].uuid,
                }].concat(ef.data.changes) });
            };
        };
    };
};