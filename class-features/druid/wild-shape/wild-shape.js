...
const actorData = target.actor.getRollData();
...
let circleForms = tactor.items.find(i => i.name.toLowerCase().includes("circle forms"));
...
const wildshapeHealing = [{
    ...
    "data": {
        "description": {
            ...
            "chat": "<p>You regain 1d8 hit points</p>\n<p><em><strong>At Higher Levels.</strong></em> When you use this feature using a spell slot of 2nd level or higher, the healing increases by 1d8 for each slot level above 1st.</p>",
            "unidentified": ""
        },
        "activation": {
            "type": "action",
        ...
        buttons: {
            change: {
                label: "Change", callback: async (html) => {
                    ...
                    let wsHealing = findPoly.items.find(i => i.name === "Combat Wild Shape Healing");
                    let keys = Object.keys(tactor.data.data.spells);
                    let updates = keys.reduce((acc, values, i) => {
                        let primaryLabel = "data.resources.primary.label";
                        let primaryValue = "data.resources.primary.value";
                        ...
                        let spellMin = `data.spells.${values}.value`;
                        let spellMax = `data.spells.${values}.max`;
                        if (combatWildShape && circleForms) {
                            acc[spellMin] = Object.values(tactor.data.data.spells)[i].value;
                            acc[spellMax] = Object.values(tactor.data.data.spells)[i].max;
                        }
					...
					if (primalStrike) {
						weapons = findPoly.items.filter((i) => i.data.type === "weapon");
						weapons.forEach((weapon) => {
							let copyItem = duplicate(weapon);
							copyItem.data.properties.mgc = true;
							findPoly.updateEmbeddedDocuments("Item", [copyItem]);
						});
					}
                }
            }
