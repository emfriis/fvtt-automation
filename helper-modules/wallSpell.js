      ...
      <div><img src="${token?.data?.img}" height="30" style="border:0px"></div>
        ...
        if (utilFunctions.isMidiActive()) {
            let itemData = {
                ...
                img: spellItem.img,
                type: "feat",
                flags: spellItem.data.flags,
                data: {
                    "activation.type": "none",
                    actionType: spellItem.data.data.actionType,
                    damage: spellItem.data.data.damage,
                    save: { dc: casterActor.data.data.attributes.spelldc, ability: spellItem.data.data.save.ability, scaling: "flat" },
                    target: { value: null, width: null, units: null, type: "creature" },
                }
            }
            let item = new CONFIG.Item.documentClass(itemData, { parent: casterActor });
            await MidiQOL.completeItemRoll(item, { showFullCard: false, createWorkflow: true, configureDialog: false, targetUuids: [token.document.uuid] });
            ...
            if (targets.length && targets.length > 0) {
                ...
                if (halfDamOnSave) {
                    for await (let targetToken of targets) {
                        // HALF DAMAGE ITEM HERE !!!
                        let itemData = {
                            name: wallItem.name,
                            img: wallItem.img,
                            type: "feat",
                            flags: wallItem.data.flags,
                            data: {
                                "activation.type": "none",
                                actionType: wallItem.data.data.actionType,
                                damage: wallItem.data.data.damage,
                                save: { dc: casterActor.data.data.attributes.spelldc, ability: wallItem.data.data.save.ability, scaling: "flat" },
                                target: { value: null, width: null, units: null, type: "creature" },
                            }
                        }
                        let item = new CONFIG.Item.documentClass(itemData, { parent: casterActor });
                        await MidiQOL.completeItemRoll(item, { showFullCard: false, createWorkflow: true, configureDialog: false, targetUuids: [targetToken.document.uuid] });
