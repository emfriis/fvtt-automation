    ...
    try {
        ...
        if (workflow.targets) {
            ...
            for (let t = 0; t < targets.length; t++) {
                ...
                // downed
                if ((tactor.data.data.attributes.hp.value === 0 || tactor.effects.find(e => ["Dead", "Defeated", "Unconscious"].includes(e.data.label) && !e.data.disabled)) && !(!workflow.item.id && workflow.item.name.toLowerCase().includes("save"))) {
                    try {
