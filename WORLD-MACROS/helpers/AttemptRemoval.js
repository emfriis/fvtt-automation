...
try {
    ...
    if (args[0] === "each" && lastArg.efData.disabled === false) {
        ...
        const type = args[3];
        let getResist = false;
        if (type === "save") getResist = targetActor.data.flags["midi-qol"]?.resilience[condition.toLowerCase()] || ((origin?.data?.data?.properties?.mgc || origin?.data?.flags?.midiProperties?.magiceffect || lastArg.efData?.flags?.magiceffect) && (targetActor.data.flags["midi-qol"]?.magicResistance.all || targetActor.data.flags["midi-qol"]?.magicResistance[args[2]])) || ((origin?.data?.type === "spell" || lastArg.efData?.flags?.spelleffect) && targetActor.data.flags["midi-qol"].spellResistance);
