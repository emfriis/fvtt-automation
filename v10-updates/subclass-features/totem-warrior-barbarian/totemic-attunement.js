            ...
            if (effect.label == "Rage" && effect.parent.uuid == args[0].actor.uuid) {
                let hook2 = Hooks.on("midi-qol.RollComplete", async (workflowNext) => {
                    if (args[0].uuid == workflowNext.uuid) {
                        ...
                        const effectData = {
                            changes: [{ key: "flags.midi-qol.disadvantage.attack.all", mode: 0, value: `actorUuid!="${args[0].actor.uuid}"&&![...workflow.targets][0].actor.effects.find(e=>e.label=="Totemic Attunement: Bear")&&!traits.ci.value.has("frightened")&&MidiQOL.canSense(canvas.tokens.get(tokenId), canvas.tokens.get("${args[0].tokenId}"))`, priority: 20 }],
                            disabled: false,
                            label: "Totemic Attunement: Bear",
                        ...
                        const auraEffect = args[0].actor.effects.find(e => e.label == "Totemic Attunement: Bear");
