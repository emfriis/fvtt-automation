try {
    if (args[0].tag == "OnUse" && args[0].macroPass == "postActiveEffects" && ["mwak", "rwak", "msak", "rsak"].includes(args[0].item.system.actionType) && args[0].actor.effects.find(e => e.label == "Rage") && (!game.combat || game.combat?.current?.tokenId == args[0].tokenId) && (!game.combat || !args[0].actor.effects.find(e => e.label == "Used Ancestral Protectors" && !e.disabled))) {
        const effectData1 = {
        ...
        if (game.combat) {
            const effectData2 = {
                ...
                icon: "icons/creatures/magical/spirit-undead-ghost-blue.webp",
                label: "Used Ancestral Protectors"
            }
		...
		const effectData = {
            ...
            icon: "icons/creatures/magical/spirit-undead-ghost-blue.webp",
            label: "Ancestral Protectors Damage Resistance"
        }
        ...
        Hooks.once("midi-qol.damageApplied", async () => {
            const effects = args[0].targets[0].actor.effects.filter(e => e.label == "Ancestral Protectors Damage Resistance").map(e => e.id);
