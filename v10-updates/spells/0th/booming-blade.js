			...
			buttons: {
				Ok: {
					...
					callback: async () => {
						...
						weaponCopy.effects.push({ name: "Booming Blade", icon: args[0].item.img, origin: args[0].item.uuid, transfer: false, disabled: false, isSuppressed: false, duration: { seconds: 7, rounds: 1 }, flags: { dae: { specialDuration: ["turnStartSource", "combatEnd"], transfer: false, stackable: "noneName" } }, changes: [{ key: "macro.execute", mode: 0, value: `Compendium.dnd-5e-core-compendium.macros.IEsMzgcrqW3AIeJ8 ${cantripDice}d8 ${args[0].workflow.defaultDamageType}`, priority: "20" }] });
