...
if (lastArg.tag === "DamageBonus") {
    ...
    if (conc) {
        const getResist = tactor.data.flags["midi-qol"]?.resilience?.blinded || tactor.data.flags["midi-qol"]?.spellResistance?.save || (tactor.data.flags["midi-qol"]?.magicResistance?.all && typeof(tactor.data.flags["midi-qol"]?.magicResistance?.all) !== "object") || tactor.data.flags["midi-qol"]?.magicResistance?.all?.con;
