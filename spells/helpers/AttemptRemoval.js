    ...
    if (game.dfreds.effectInterface.hasEffectApplied(condition, targetToken.uuid)) {
        ...
        buttons: {
            ...
            callback: async () => {
                ...
                const rollOptions = { chatMessage: false, fastForward: true };
                ui.notifications.warn(`${type}, ${targetUuid}, ${ability}, ${rollOptions}`);
                ...
                if (roll.total < saveDc) ChatMessage.create({ content: `${targetToken.name} fails the ${type} for ${item.name}, still has the ${condition} condition.` });
                }
            },
            },
            two: {
