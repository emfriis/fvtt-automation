...
if (args[0] === "on" || args[0] === "each") {
    ...
    if (!tactor.effects.find(e => e.data.label === "Restrained" && e.data.origin === lastArg.efData.origin)) {
        ...
        if (applyCondition) await applyCondition.execute("ApplyCondition", lastArg.Id, "save", "Restrained", args[1], "dex", "", "", "magiceffect", "spelleffect", `${args[1]},str,abil,opt`, "startEveryTurn", lastArg.efData.origin);
    }
} else if (args[0] === "off") {
